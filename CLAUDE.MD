...
- Test files that might be using it

### Conditional Rule Loading
Claude Code supports conditional rules. Put extra rules in `.claude/rules/*.md`
with YAML frontmatter specifying which files they apply to. Those rules
only activate when editing matching files (perfect for test files, migrations,
etc.).

Core Principles (always active):
- Simplicity first
- Root cause, never band-aid
- Exija elegância nas mudanças grandes
- O disco é sua memória extra
- Nunca confie na memória depois de 10 mensagens